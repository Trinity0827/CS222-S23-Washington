...
class MyDictionary():
    ...
    def get(self,k): #pass --> objext has no atrribute 'get
...
class TestMyDictionary(unittest.TestCase):
    def setUp(self):
        self.myDictionary = MyDictionary() #its going to fail because no function for myDictionary
    ...
    def test_getNonExistentKey(self):
        result = self.myDictionary.get('002') # the dictionary is empty so it can get anything 
